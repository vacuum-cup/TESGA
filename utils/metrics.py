...
class micro():
    ...
    def count_re_num(self, re_pred, re_label):
        # _ ,pred_ids = torch.max(re_pred,dim=-1)
        
        # re_pred_num = len(pred_ids)
        # re_gold_num = len(re_label)

        # re_right = torch.where(pred_ids == re_label,torch.ones_like(pred_ids),torch.zeros_like(pred_ids))
        # re_right_num = re_right.sum().item()
        # return [re_pred_num, re_gold_num, re_right_num]
        # re_pred = torch.where(re_pred>=0.5, torch.ones_like(re_pred),
        #                             torch.zeros_like(re_pred))
        # re_pred_num = re_pred.sum().item()
        # re_gold_num = re_label.sum().item()
        ...
        return [pred_positive, gold_positive, correct_positive]
...
class loss(nn.Module):
    def __init__(self):
        ...
        self.loss_re_ = nn.CrossEntropyLoss(reduction="sum")

    def forward(self, ner_pred, ner_label, re_pred, re_label):
        if ner_label ==None:
            # seq_len = re_pred.size(1)
            loss = self.loss_re_(re_pred, re_label)
        elif re_label == None:
            seq_len = ner_pred.size(1)
            loss = self.loss_ner(ner_pred, ner_label)/seq_len
        else:
            seq_len = ner_pred.size(1)
