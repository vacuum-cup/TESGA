...
class Ner(nn.Module):
    def __init__(self, args, ner2idx):
        ...
        self.hid2hid = nn.Linear(self.hidden_size * 5, self.hidden_size*2)
        self.hid2tag = nn.Linear(self.hidden_size*2, len(ner2idx))

        self.elu = nn.ELU()  # 激活函数
        self.n = nn.Linear(self.hidden_size*2, self.hidden_size)
        self.n1 = nn.Linear(768, self.hidden_size)
        self.ln = nn.LayerNorm(self.hidden_size*2)  # 对 -1维度进行当前batch归一化

        self.dropout = nn.Dropout(args.dropout)



    def forward(self, h_ner1, h_ner2,g_f, mask,token_pos=None):
        length, batch_size, _ = h_ner2.size()
        ...
        # h_global = h_global.unsqueeze(0).repeat(h_ner2.size(0), 1, 1)   # max_len*batch_size*hidden_size
        # h_global = h_global.unsqueeze(0).repeat(h_ner2.size(0), 1, 1, 1)   # max_len*max_len*batch_size*hidden_size
        g_f = torch.tanh(self.n1(g_f))
        g_f = g_f.unsqueeze(0).repeat(length, 1, 1)
        g_f = g_f.unsqueeze(0).repeat(length, 1, 1, 1)

        st = h_ner2.unsqueeze(1).repeat(1, length, 1, 1)
        en = h_ner2.unsqueeze(0).repeat(length, 1, 1, 1)
        
        st1 = h_ner1.unsqueeze(1).repeat(1, length, 1, 1)
        en1 = h_ner1.unsqueeze(0).repeat(length, 1, 1, 1)

        ner = torch.cat((st, en,st1,en1,g_f), dim=-1)
...
class RE(nn.Module):
    def __init__(self, args, re2idx):
        ...
        # self.hid2hid = nn.Linear(self.hidden_size*4+768, self.hidden_size)
        self.hid2rel = nn.Linear(self.hidden_size*4+768, self.relation_size)
        self.elu = nn.ELU()

        # self.r = nn.Linear(self.hidden_size, self.hidden_size)
        self.ln = nn.LayerNorm(self.hidden_size)
    ...
    def forward(self, h_re1, h_re2, global_feature, token_pos=None, s_o_indes=None):
        length, batch_size, _ = h_re2.size()

        h_global = torch.cat((h_re1, h_re2), dim=-1)
        
        ents = torch.stack([torch.cat((h_global[pos[0]][i],h_global[pos[1]][i]))  for i,pos in enumerate(s_o_indes)],dim=0) 
        
        re = torch.cat((global_feature,ents), dim=-1)
        
        # re = self.ln(self.hid2hid(re))
        # re = torch.tanh(self.dropout(re))
        re = self.hid2rel(re)
...
class TESGA(nn.Module):
    ...
    def forward(self, x, images, sents, sim_score, mask,token_pos = None,s_o_indes = None):
...
class MCRE(nn.Module):
    def __init__(self, args, input_size, ner2idx, rel2idx, s_o_indes = None):
        ...
        self.img_freature_transform = LinearDropConnect(input_size, 2*args.hidden_size, bias=True, dropout= args.dropconnect)
        # self.img_freature_prompt = LinearDropConnect(input_size, 2*args.hidden_size, bias=True, dropout= args.dropconnect)
    ...
    def forward(self, x, images, sents, sim_score, mask,token_pos = None,s_o_indes = None):
        # img_feature1 = self.img_model(images, sents, mode='image')
        # img_f1_mean = torch.mean(img_feature1, dim=1)
        # im_f1_max = torch.max(img_feature1, dim=1)[0]
        # bsz = img_f1_mean.size(0)
        # prompt_guidk = img_f1_mean.reshape(bsz,12,64)
        # prompt_guidv = img_f1_max.reshape(bsz,12,64)
        # prompt_guids = (prompt_guidk.unsqueeze(1).repeat(1,12,1,1),prompt_guidv.unsqueeze(1).repeat(1,12,1,1))
        # prompt_guids = [prompt_guids for i in range(12)]
        
        x = self.tokenizer(x, return_tensors="pt",padding='longest',is_split_into_words=True).to(device)   # 分词

        # prompt_guids_mask = torch.ones((bsz, 12)).to(device)   # bsz ,16
        # prompt_attention_mask = torch.cat((prompt_guids_mask, attention_mask), dim=1)  # 掩码扩展 
        
        output = self.bert(**x)  
        
        x = output[0]# size  batch size * max_len * 768
        text_global_feature = output[1]
        
        # img_feature = self.img_model(images, sents, mode='image')
        
        # img_feature = torch.tanh(self.img_freature_transform(img_feature)) 
        # imf_1,imf_2=img_feature.chunk(2,-1)
        
        img_feature = self.img_model(images, sents, mode='multimodal')
        img_feature = self.img_freature_transform(img_feature)
        # imf_1 = torch.mean(imf_1, dim=1)
        img_feature = torch.max(img_feature, dim=1)[0]
        imf_1,imf_2=img_feature.chunk(2,-1)
        ...
        h_re1,_ = self.get_re_ls_feature(h1)
        h_re2 = self.crosstransRE(imf_2,self.selftransRE(h_re1,h2),sim_score)
        
        if(token_pos!= None):
            token_pos = token_pos.transpose(0,1)
        
        re_core = self.re(h_re1, h_re2, text_global_feature,token_pos,s_o_indes)
        return None, re_core
...
class MCNER(nn.Module):
    ...
    def forward(self, x, images, sents, sim_score, mask,token_pos = None,s_o_indes = None):
        ...
        x = self.tokenizer(x, return_tensors="pt",padding='longest',is_split_into_words=True).to(device)   # 分词
        output = self.bert(**x)  # size  batch size * max_len * 768
        x = output[0]
        text_g_f = output[1]
        ...
        ner_score = self.ner(h_ls, h_ner2, text_g_f,mask,token_pos)
